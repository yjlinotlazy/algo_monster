"""Best-effort per-request telemetry shared by the app server."""
...
def route(target: str) -> str:
    ...
    if path.startswith("/api/"):
        return "/".join(path.split("/")[:3]) + ("/:path" if len(path.split("/")) > 3 else "")
    return _INTEGER_PATH.sub("/:id", path)


def loggable(path: str) -> bool:
    return path != "/favicon.ico" and not path.startswith(("/assets/", "/static/"))
...
class RequestLogger:
    ...
    def record(self, *, target: str, method: str, status: int, request_size: int, response_size: int, started_at: float) -> None:
        ...
        try:
            ...
            file = self.directory / f"{datetime.fromtimestamp(started_at).date().isoformat()}.jsonl"
            with _LOCK, file.open("a", encoding="utf-8") as stream:
